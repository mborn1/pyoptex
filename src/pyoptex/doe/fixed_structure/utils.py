...
Parameters = namedtuple(
    "Parameters", "fn factors nruns effect_types effect_levels grps grp_runs ratios coords prior colstart Zs Vinv"
)
...
class RandomEffect(__RandomEffect__):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        assert np.all(np.any(np.expand_dims(Z, 0) == np.expand_dims(np.arange(max_Z), 1), axis=1)), (
...
__Factor__ = namedtuple(
    "__Factor__", "name re type min max levels coords", defaults=(None, None, "cont", -1, 1, None, None)
)
