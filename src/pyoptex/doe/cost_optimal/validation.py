...
def validate_state(state, params, eps=1e-6):
    ...
    assert np.all(state.Y[: len(params.prior)] == params.prior), (
        f"(validation) Prior is no longer correct: {state.Y[: len(params.prior)] - params.prior}"
    )
    ...
    assert all((Zs[i] is None and state.Zs[i] is None) or np.all(Zs[i] == state.Zs[i]) for i in range(len(Zs))), (
        f"(validation) Grouping matrices Zs are wrong: {Zs}, {state.Zs}"
    )

    # Make sure every set of ratios has a Vinv attached
    assert params.ratios.shape[0] == len(state.Vinv), (
        f"(validation) Number of variance sets does not match the numbe of Vinv matrices: {params.ratios.shape[0]}, {len(state.Vinv)}"
    )

    # Validate Vinv
    for i in range(len(state.Vinv)):
        vinv = np.linalg.inv(obs_var_from_Zs(state.Zs, len(state.Y), params.ratios[i]))
        assert np.all(np.abs(state.Vinv[i] - vinv) < eps), (
            f"(validation) Vinv[{i}] does not match: {np.linalg.norm(state.Vinv[i] - vinv)}"
        )
    ...
    assert np.all(state.max_cost == max_cost), (
        f"(validation) The maximum cost (budget) does not match: {state.max_cost}, {max_cost}"
    )

    # Validate cost indices
    assert all(np.all(costs[i][2] == state.costs[i][2]) for i in range(len(costs))), (
        f"(validation) The cost indices do not match: {[idx for _, _, idx in state.costs]}, {[idx for _, _, idx in costs]}"
    )
