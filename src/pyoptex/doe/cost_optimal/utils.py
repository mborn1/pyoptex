...
__Factor__ = namedtuple(
    "__Factor__", "name grouped ratio type min max levels coords", defaults=(None, True, 1, "cont", -1, 1, None, None)
)
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if isinstance(self.ratio, (tuple, list, np.ndarray)):
            assert all(r >= 0 for r in self.ratio), (
                f"Variance ratio of factor {self.name} must be larger than or equal to zero, but is {self.ratio}"
            )
        else:
            assert self.ratio >= 0, (
                f"Variance ratio of factor {self.name} must be larger than or equal to zero, but is {self.ratio}"
